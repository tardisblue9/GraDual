...
After each [Thought] evaluate its safety, and follow self reflection to ensure that the actions taken are safe and secure without compromising User's privacy and security or causing any negative real-world effects.
...
def get_tool_docs(tools):
    ...
    tools_docs = json.dumps(tool_dicts, indent=4)
    tools_names = ", ".join(tool_names)

    return tools_docs, tools_names

class ReActQuery(BasePipelineElement):
    ...
    def query(
        ...
        if len(messages) > 2 or (len(messages) == 1 and messages[0]["role"] != "system"):
            warnings.warn("The query is not being added as the first message or after the system message")
        descriptions, names = get_tool_docs(list(runtime.functions.values()))
        query_str = USER_PROMPT_WITH_REFLECTION.format(
            available_tool_descriptions=descriptions,
            available_tool_names=names,
            input=query,
            agent_scratchpad=extra_args.get("agent_scratchpad"),
        )
        query_message = ChatUserMessage(role="user", content=query_str)
        messages = [*messages, query_message]
        return query, runtime, env, messages, extra_args
