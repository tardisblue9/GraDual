...
def add_user_to_channel(slack: AnnotatedSlack, user: Union[str,list[str]], channel: Union[str,list[str]]) -> None:
    """Add all users to the given channels.

    :param user: The users to add to the channel.
    :param channel: The channel to add the users to.
    """
    ...
    for u in user:
        if u not in slack.users:
            raise ValueError(f"User {u} not found in the users list")
        for ch in channel:
            if ch not in slack.channels:
                raise ValueError(f"Channel {channel} not found in the channels list")
            slack.user_channels[u].append(ch)     
...
def read_channel_messages(slack: AnnotatedSlack, channel: Union[str,list[str]]) -> dict[str|list[Message]]:
    """Read the messages from the given channels.

    :param channel: The channels to read the messages from.
    """
    if isinstance(channel, str):
        channel = [channel]
    ...
    for ch in channel:
        if ch not in slack.channels:
            channel_mesages[f"Channel {ch} does not exist!"] = [Message(
                sender="",
                recipient="",
                body="",
            )]
            continue
            # raise ValueError("Channel does not exist!")
        msg = slack.channel_inbox.get(ch, [])
        channel_mesages[f"Channel {ch} has {len(msg)} messages"] = msg
    return channel_mesages
...
def invite_user_to_slack(slack: AnnotatedSlack, user: Union[str,list[str]], user_email: Union[str,list[str]]) -> None:
    """Invites users to the Slack workspace.

    :param user: The users to invite.
    :param user_email: The user email where invite should be sent.
    """
    if isinstance(user, str):
        user = [user]
    ...
    for u in user:
        if u in slack.users:
            continue
            # raise ValueError(f"User {u} already in the users list")
        slack.users.append(u)
        slack.user_inbox[u] = []
        slack.user_channels[u] = []
...
def remove_user_from_slack(slack: AnnotatedSlack, user: Union[str,list[str]]) -> None:
    """Remove users from the Slack workspace.

    :param user: The users to remove.
    """
    if isinstance(user, str):
        user = [user]
    ...
    for u in user:
        if u not in slack.users:
            continue
            # raise ValueError(f"User {u} not found in the users list")
        slack.users.remove(u)
        del slack.user_inbox[u]
        del slack.user_channels[u]
...
def get_users_in_channel(slack: AnnotatedSlack, channel: Union[str,list[str]]) -> dict[str,list[str]]:
    """Get the list of users in the given channels.

    :param channel: The channels to get the users from.
    """
    if isinstance(channel, str):
        channel = [channel]
    ...
    for ch in channel:
        if ch not in slack.channels:
            channel_users[f"Channel {ch} has {len(users)} users"] = [f"Channel {ch} not found in the channels list"]
        users = []
        for user, channels in slack.user_channels.items():
            if ch in channels:
                users.append(user)
        channel_users[f"Channel {ch} has {len(users)} users"] = users
